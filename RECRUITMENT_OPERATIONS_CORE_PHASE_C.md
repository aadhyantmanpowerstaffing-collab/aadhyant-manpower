...
Checkpoint 035 verifies signatures, authorization, grants, privacy text, bounded history, retained canonical baselines, and deterministic authorized/unauthorized list/detail RPC calls inside a rollback scope. It passed in NONPROD and rolled back cleanly; the provider-controlled read-only residue gate subsequently passed with zero deterministic Phase-C fixture residue and `transaction_read_only = on`. W7C campaign → recipient → INTERESTED → Candidate → Application attribution remains unchanged.
