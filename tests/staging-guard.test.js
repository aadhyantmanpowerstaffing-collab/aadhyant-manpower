...
function canonicalManifest() {
  ...
  const migrations = fs.readdirSync(migrationRoot)
    .filter((name) => /^(00[7-9]|01[0-9]|02[0-9]|03[0-9])_.*\.sql$/.test(name))
    .sort();
  const numbers = migrations.map((name) => Number(name.slice(0, 3)));
  assert.deepEqual(numbers, Array.from({ length: 33 }, (_, index) => index + 7));
...
test('guard manifest is exact through migration 039 and excludes migration 040+', () => {
  assert.match(guard, /03\[0-9\]/);
  assert.match(guard, /\$expectedNumbers = @\(7\.\.39\)/);
  assert.match(guard, /\$files\.Count -ne 34/);
  assert.match(guard, /schema\.sql plus exactly migrations 007-039/);
  assert.doesNotMatch(guard, /\$expectedNumbers = @\(7\.\.40\)|007-040/);
  assert.match(readme, /migrations 007.039 \(34 files total/);
  assert.match(readme, /Migration 040 and later files are excluded/);
});
...
test('canonical manifest ordering and aggregate are deterministic', () => {
  ...
  assert.equal(first.lines.at(-1).split('=')[0], 'supabase/migrations/039_unified_vacancy_review_workflow.sql');
  assert.equal(first.digest, 'b83b3dd9794a8c5c8a97410251731bceee60c946e83ccd699224de45d15372c2');
  assert.equal(second.digest, first.digest);
});

test('guard self-tests cover missing, duplicate, future migration, and exact HEAD refusal', () => {
  assert.match(guard, /039_fixture\.sql/);
  assert.match(guard, /039_duplicate\.sql/);
  assert.match(guard, /040_future\.sql/);
