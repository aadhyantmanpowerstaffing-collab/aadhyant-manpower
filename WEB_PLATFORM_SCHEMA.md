...
Migration 023 reuses canonical Candidate and recruitment records. Candidate ownership resolves from `auth.uid()` through one active Candidate platform/linkage row. New normalized `candidate_documents` and `candidate_onboarding_details` entities hold private document metadata and restricted joining details without adding a parallel recruitment workflow. The private `candidate-private` Storage bucket accepts declared PDF/JPEG/PNG up to 10 MB. Candidate self-service requires both active, unambiguous Candidate linkage and a matching Auth UID path. Bootstrap Admin, Super Admin, and Admin receive separate SELECT-only verifier access; Company, Contractor, Recruiter, Operations, unrelated Candidate, and anonymous identities remain denied.
...
Migration 024 preserves the Candidate interview RPC's text return contract by explicitly casting canonical `interviews.interview_round` from `smallint` to text; its authorization and privacy contract is unchanged. Migration 025 removes UID-folder-only Storage authorization, adds the Candidate-linkage requirement and narrow verifier read policy, and adds distinct review-start/final-review attribution for the controlled Uploaded, Under Verification, Verified, and Re-upload Required lifecycle. The Admin UI mirrors that lifecycle: Uploaded exposes View Securely and Start Review; Under Verification exposes View Securely, Mark Verified, and Request Re-upload; Verified and Re-upload Required expose View Securely only. Start Review invokes the canonical review RPC with `under_verification`, and successful transitions reload the server projection.

Final focused NONPROD browser evidence proves Uploaded -> Start Review -> Under Verification -> Verified, correct review-start/reviewed/verified actor and timestamp attribution, and final-state action removal. The complete frontend result is 121/121. The two-user focused fixture, review audit rows, and local browser artifacts were removed with zero Auth identity/session, database, audit, Storage, orphan, or cross-tenant residue. W6 is technically complete locally; normal non-force push and remote HEAD verification remain separately authorized closure steps.

Full Aadhaar is not persisted: W6 retains a deterministic SHA-256 equality fingerprint and last four digits. SHA-256 hashing is not encryption and its enumerable input space is a production risk; key-managed HMAC/tokenization is required before real production Aadhaar intake. Bank account reads remain masked and Candidate-owned. Existing UAN/ESIC capture provides conditional validation only and does not claim government verification, generation, registration, or filing. Declared MIME/extension/size checks do not provide magic-byte inspection, malware scanning, or quarantine; those document controls and operational monitoring/retention remain production prerequisites.
