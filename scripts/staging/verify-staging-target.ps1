...
function Get-MigrationManifestHash {
    ...
    $migrations = @(
        Get-ChildItem -LiteralPath (Join-Path $RepositoryRoot 'supabase\migrations') -File -Filter '*.sql' |
            Where-Object { $_.Name -match '^(00[7-9]|01[0-9]|02[0-9]|03[0-9]|04[0-3])_' } |
            Sort-Object Name
    )
    $expectedNumbers = @(7..43)
    ...
    if (@($actualNumbers | Group-Object | Where-Object Count -ne 1).Count -ne 0) {
        Stop-Guard 'Duplicate migration number detected in the required 007-043 range.'
    }
    if (@(Compare-Object $expectedNumbers $actualNumbers -SyncWindow 0).Count -ne 0) {
        Stop-Guard 'Expected exactly one migration for each number 007-043.'
    }
    $files = @(
        Join-Path $RepositoryRoot 'supabase\schema.sql'
        $migrations | Select-Object -ExpandProperty FullName
    )
    if ($files.Count -ne 38) { Stop-Guard 'Expected schema.sql plus exactly migrations 007-043.' }
...
function Invoke-GuardAssertionTests {
    ...
    try {
        ...
        [IO.File]::WriteAllText((Join-Path $migrationRoot '044_future.sql'), 'future-migration')
        if ((Get-MigrationManifestHash -RepositoryRoot $fixtureRoot) -cne $firstHash) {
            throw 'Migration 044 was not excluded from the approved manifest.'
        }

        [IO.File]::Delete((Join-Path $migrationRoot '043_fixture.sql'))
        Assert-GuardRefusal -ExpectedMessage 'Expected exactly one migration for each number 007-043.' -Action {
            Get-MigrationManifestHash -RepositoryRoot $fixtureRoot
        }
        [IO.File]::WriteAllText((Join-Path $migrationRoot '043_fixture.sql'), 'migration-43')
        [IO.File]::WriteAllText((Join-Path $migrationRoot '043_duplicate.sql'), 'duplicate-migration-43')
        Assert-GuardRefusal -ExpectedMessage 'Duplicate migration number detected in the required 007-043 range.' -Action {
            Get-MigrationManifestHash -RepositoryRoot $fixtureRoot
        }
