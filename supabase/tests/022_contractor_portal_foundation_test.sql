...
do $$ declare ctx record; created record; begin
  ...
  if exists(select 1 from public.employer_requirements r where r.id=created.id and (r.requirement_stage<>'draft' or r.requirement_visibility<>'private')) then raise exception 'Submission bypassed review'; end if;
...
do $$ begin
  ...
  begin perform public.get_contractor_portal_vacancy((select r.id from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001'));raise exception 'Contractor B read Contractor A vacancy';exception when raise_exception then if sqlerrm='Contractor B read Contractor A vacancy' then raise;end if;end;
  begin perform public.manage_contractor_portal_vacancy(p_action=>'update',p_requirement_id=>(select r.id from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001'),p_client_name=>'Spoof',p_job_role=>'Fitter',p_job_location=>'Pune',p_required_headcount=>1);raise exception 'Contractor B mutated Contractor A vacancy';exception when raise_exception then if sqlerrm='Contractor B mutated Contractor A vacancy' then raise;end if;end;
end $$;
...
do $$ declare rid uuid; reviewed record;begin
  ...
  select r.id into rid from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001';
  perform public.review_contractor_vacancy(rid,'request_correction','Clarify the shift details');
end $$;

select set_config('request.jwt.claim.sub','86000000-0000-0000-0000-000000000001',true);
do $$ declare rid uuid; updated record; detail jsonb;begin
  select r.id into rid from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001';
...
do $$ declare rid uuid;begin select r.id into rid from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001';begin perform public.review_contractor_vacancy(rid,'approve',null);raise exception 'Staff Recruiter approved vacancy';exception when raise_exception then if sqlerrm='Staff Recruiter approved vacancy' then raise;end if;end;end $$;
select set_config('request.jwt.claim.sub','86000000-0000-0000-0000-000000000011',true);
do $$ declare rid uuid;begin select r.id into rid from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001';begin perform public.review_contractor_vacancy(rid,'approve',null);raise exception 'Staff Operations approved vacancy';exception when raise_exception then if sqlerrm='Staff Operations approved vacancy' then raise;end if;end;end $$;

select set_config('request.jwt.claim.sub','86000000-0000-0000-0000-000000000006',true);
do $$ declare rid uuid; reviewed record;begin
  select r.id into rid from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001';select * into reviewed from public.review_contractor_vacancy(rid,'approve','Approved synthetic vacancy');if reviewed.submission_status<>'approved' or reviewed.requirement_stage<>'open' or reviewed.requirement_visibility<>'assigned' then raise exception 'Internal approval bridge failed';end if;if not exists(select 1 from public.list_recruitment_requirements(null,'open',50,0) r where r.id=rid) then raise exception 'Approved vacancy not visible to W3';end if;end $$;
...
select '86000000-0000-0000-0003-000000000001','86000000-0000-0000-0002-000000000001',r.id,'admin','selected','86000000-0000-0000-0000-000000000006' from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001';
...
do $$ declare app record;detail jsonb;metrics record;begin select * into app from public.list_contractor_portal_applications();if app.candidate_name<>'W5 Safe Candidate' then raise exception 'Safe application projection failed';end if;detail:=public.get_contractor_portal_application(app.application_id);if detail? 'mobile' or detail?'whatsapp_number' or detail?'internal_notes' or detail?'candidate_id' then raise exception 'Contractor candidate privacy failed';end if;if(select count(*) from public.list_contractor_portal_interviews())<>1 or(select count(*) from public.list_contractor_portal_joinings())<>1 then raise exception 'Read-only progress projection failed';end if;select * into metrics from public.get_contractor_dashboard_metrics();if metrics.approved_active<>1 or metrics.applications<>1 or metrics.interviews<>1 or metrics.selected<>1 or metrics.joining_pending<>0 then raise exception 'Contractor dashboard failed';end if;
  ...
  begin perform public.manage_contractor_portal_vacancy(p_action=>'update',p_requirement_id=>(select r.id from public.employer_requirements r where r.created_by_user_id='86000000-0000-0000-0000-000000000001'),p_client_name=>'Synthetic Client Worksite',p_job_role=>'Fitter',p_job_location=>'Chennai',p_required_headcount=>5);raise exception 'Approved vacancy regressed';exception when raise_exception then if sqlerrm='Approved vacancy regressed' then raise;end if;end;
end $$;
