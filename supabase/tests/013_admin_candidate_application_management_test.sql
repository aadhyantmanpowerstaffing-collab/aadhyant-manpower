...
values('54000000-0000-0000-0000-000000000001','53000000-0000-0000-0000-000000000001','52000000-0000-0000-0000-000000000001','direct','applied');
...
do $$
begin
  if exists(select 1 from r12_candidate_before b left join public.candidates c using(id) where c.id is null or row(c.*) is distinct from row(b.*)) then raise exception 'Candidate master changed'; end if;
  if exists(select 1 from r12_requirement_before b left join public.employer_requirements r using(id) where r.id is null or row(r.*) is distinct from row(b.*)) then raise exception 'Requirement changed'; end if;
