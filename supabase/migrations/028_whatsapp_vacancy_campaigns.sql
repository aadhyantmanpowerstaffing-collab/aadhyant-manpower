...
create table public.whatsapp_campaigns (
  ...
  constraint whatsapp_campaign_state_dates_check check (
    (campaign_status in ('draft','audience_ready') or approved_at is not null)
    and (campaign_status not in ('queued','sending','completed') or queued_at is not null)
...
create index whatsapp_campaign_recipients_outbound_idx on public.whatsapp_campaign_recipients(outbound_message_id) where outbound_message_id is not null;
...
returns boolean language sql immutable set search_path='' as $$
  select jsonb_typeof(coalesce(p_criteria,'{}'::jsonb))='object'
    ...
    and not exists(select 1 from jsonb_each(coalesce(p_criteria,'{}'::jsonb)) e
      where (e.key='avoid_existing_application' and jsonb_typeof(e.value)<>'boolean')
...
begin
  ...
  select c.id,left(c.full_name,1)||repeat('•',greatest(1,least(length(c.full_name)-1,12))),c.highest_qualification,c.specialization,c.candidate_type,
    ...
    (a.id is not null),
    (wc.id is not null and wc.resolution_status not in ('blocked','ambiguous') and wc.marketing_consent_status='opted_in'
      and c.profile_status='active' and c.status<>'inactive' and (a.id is null or coalesce((criteria->>'avoid_existing_application')::boolean,true)=false)),
    case when c.profile_status<>'active' or c.status='inactive' then 'inactive_candidate'
      ...
      when wc.resolution_status='ambiguous' then 'duplicate_contact' when wc.marketing_consent_status='opted_out' then 'opted_out'
...
create function public.admin_create_whatsapp_campaign(p_requirement_id uuid,p_campaign_name text,p_template_name text,
  p_template_language text,p_template_version text,p_criteria jsonb default '{}'::jsonb)
returns uuid language plpgsql security definer set search_path='' as $$
declare result uuid; criteria jsonb:=coalesce(p_criteria,'{}'::jsonb); actor uuid:=(select auth.uid());
begin
  if not (select private.can_manage_whatsapp_campaigns()) then raise exception 'WhatsApp campaign Admin access is required'; end if;
  ...
  if not (select private.whatsapp_campaign_criteria_safe(criteria)) then raise exception 'Campaign criteria are unsupported'; end if;
  insert into public.whatsapp_campaigns(requirement_id,campaign_name,purpose,consent_class,template_name,template_language,template_version,criteria,created_by)
  values(p_requirement_id,btrim(p_campaign_name),'vacancy_campaign','marketing',btrim(p_template_name),btrim(p_template_language),btrim(p_template_version),criteria,actor)
...
declare campaign public.whatsapp_campaigns%rowtype; cid uuid; contact public.whatsapp_contacts%rowtype; included_count integer:=0; actor uuid:=(select auth.uid());
begin
  if not (select private.can_manage_whatsapp_campaigns()) then raise exception 'WhatsApp campaign Admin access is required'; end if;
  ...
  for cid in select distinct input.candidate_id from unnest(coalesce(p_included_candidate_ids,'{}'::uuid[])) as input(candidate_id) loop
    if not exists(select 1 from public.candidates c where c.id=cid and c.profile_status='active' and c.status<>'inactive') then raise exception 'Included Candidate is not active'; end if;
    select * into contact from public.whatsapp_contacts where candidate_id=cid and provider='whatsapp' for share;
    if not found or contact.resolution_status in ('blocked','ambiguous') or contact.marketing_consent_status<>'opted_in'
...
declare campaign public.whatsapp_campaigns%rowtype; actor uuid:=(select auth.uid()); count_ready integer;
begin
  if not (select private.can_manage_whatsapp_campaigns()) then raise exception 'WhatsApp campaign Admin access is required'; end if;
  select * into campaign from public.whatsapp_campaigns where id=p_campaign_id for update;
  if not found or campaign.campaign_status<>'audience_ready' then raise exception 'Audience-ready campaign is required'; end if;
  if not exists(select 1 from public.employer_requirements where id=campaign.requirement_id and requirement_stage='open') then raise exception 'Open requirement was not found'; end if;
  ...
  select count(*) into count_ready from public.whatsapp_campaign_recipients cr join public.whatsapp_contacts wc on wc.id=cr.whatsapp_contact_id
  where cr.campaign_id=campaign.id and cr.recipient_status='included' and wc.marketing_consent_status='opted_in'
    and wc.resolution_status not in ('blocked','ambiguous') and private.whatsapp_contact_allows_purpose(wc.id,'marketing');
  if count_ready=0 or count_ready<>(select count(*) from public.whatsapp_campaign_recipients where campaign_id=campaign.id and recipient_status='included') then raise exception 'Campaign audience is empty or suppressed'; end if;
...
returns integer language plpgsql security definer set search_path='' as $$
declare campaign public.whatsapp_campaigns%rowtype; recipient public.whatsapp_campaign_recipients%rowtype; req public.employer_requirements%rowtype; outbound_id uuid; queued_count integer:=0; actor uuid:=(select auth.uid());
begin
  if not (select private.can_manage_whatsapp_campaigns()) then raise exception 'WhatsApp campaign Admin access is required'; end if;
  select * into campaign from public.whatsapp_campaigns where id=p_campaign_id for update;
  if not found then raise exception 'Campaign was not found'; end if;
  if campaign.campaign_status='queued' then return (select count(*) from public.whatsapp_campaign_recipients where campaign_id=campaign.id and recipient_status='queued'); end if;
  ...
  if queued_count=0 then raise exception 'Campaign has no included recipients'; end if;
  update public.whatsapp_campaigns set campaign_status='queued',queued_at=clock_timestamp() where id=campaign.id;
  insert into public.audit_logs(actor_user_id,actor_type,action,entity_type,entity_id,source,metadata)
  values(actor,'staff','whatsapp.campaign_queued','whatsapp_campaign',campaign.id,'admin',jsonb_build_object('queued_count',queued_count));
  return queued_count;
end;
$$;
...
returns table(campaign_id uuid,campaign_name text,requirement_id uuid,requirement_code text,job_role text,campaign_status text,
  created_at timestamptz,audience_count bigint,queued_count bigint,sent_count bigint,delivered_count bigint,read_count bigint,failed_count bigint)
language plpgsql stable security definer set search_path='' as $$
...
begin
  ...
  return query select c.id,c.campaign_name,c.requirement_id,r.requirement_code,r.job_role,c.campaign_status,c.created_at,
    ...
    count(o.id) filter(where o.state='sent'),count(o.id) filter(where o.state='delivered'),count(o.id) filter(where o.state='read'),count(o.id) filter(where o.state='failed')
...
begin
  ...
  select jsonb_build_object('id',c.id,'campaign_name',c.campaign_name,'campaign_status',c.campaign_status,'requirement_id',r.id,
    ...
    'created_at',c.created_at,'approved_at',c.approved_at,'queued_at',c.queued_at,
...
revoke all on function public.admin_preview_whatsapp_campaign_audience(uuid,jsonb,integer,integer),
  public.admin_create_whatsapp_campaign(uuid,text,text,text,text,jsonb),
  public.admin_freeze_whatsapp_campaign_audience(uuid,uuid[],uuid[],integer),public.admin_approve_whatsapp_campaign(uuid),
  public.admin_queue_whatsapp_campaign(uuid),public.admin_cancel_whatsapp_campaign(uuid),
