...
declare v_digits text:=pg_catalog.regexp_replace(pg_catalog.coalesce(p_value,''),'[^0-9]','','g');
begin
  if pg_catalog.length(v_digits)=12 and pg_catalog.left(v_digits,2)='91' then v_digits:=pg_catalog.right(v_digits,10); end if;
...
begin
  if v_status in ('accepted','sent') then
    update public.whatsapp_outbound_messages set state='sent',sent_at=pg_catalog.coalesce(sent_at,v_now),lease_owner=null,lease_expires_at=null,updated_at=pg_catalog.clock_timestamp()
      where id=p_outbound_message_id and state in ('queued','sending','sent');
  elsif v_status='delivered' then
    update public.whatsapp_outbound_messages set state='delivered',sent_at=pg_catalog.coalesce(sent_at,v_now),delivered_at=pg_catalog.coalesce(delivered_at,v_now),lease_owner=null,lease_expires_at=null,updated_at=pg_catalog.clock_timestamp()
      where id=p_outbound_message_id and state in ('queued','sending','sent','delivered','failed');
  elsif v_status='read' then
    update public.whatsapp_outbound_messages set state='read',sent_at=pg_catalog.coalesce(sent_at,v_now),delivered_at=pg_catalog.coalesce(delivered_at,v_now),read_at=pg_catalog.coalesce(read_at,v_now),lease_owner=null,lease_expires_at=null,updated_at=pg_catalog.clock_timestamp()
      where id=p_outbound_message_id and state<>'read';
  elsif v_status='failed' then
    update public.whatsapp_outbound_messages set state='failed',failed_at=pg_catalog.coalesce(failed_at,v_now),lease_owner=null,lease_expires_at=null,updated_at=pg_catalog.clock_timestamp()
      where id=p_outbound_message_id and state in ('queued','sending','sent');
...
begin
  ...
  on conflict(provider,provider_address) do update set last_inbound_at=excluded.last_inbound_at,
    candidate_id=case when whatsapp_contacts.resolution_status='blocked' then whatsapp_contacts.candidate_id else excluded.candidate_id end,
...
begin
  ...
  values(p_webhook_event_id,p_contact_id,pg_catalog.btrim(p_provider_message_id),p_provider_timestamp,
    pg_catalog.lower(pg_catalog.btrim(p_message_type)),p_safe_text,p_action_id,p_correlation_key,p_redacted_response,'normalized')
...
returns uuid language plpgsql security definer set search_path = '' as $$
declare v_contact public.whatsapp_contacts%rowtype; v_id uuid;
begin
  select * into v_contact from public.whatsapp_contacts where id=p_contact_id for share;
  if not found or not (select private.whatsapp_contact_allows_purpose(p_contact_id,p_consent_class)) then raise exception 'WhatsApp contact is suppressed for this purpose'; end if;
  ...
  values(p_contact_id,p_candidate_id,p_requirement_id,pg_catalog.btrim(p_purpose),pg_catalog.lower(pg_catalog.btrim(p_consent_class)),
    pg_catalog.lower(pg_catalog.btrim(p_template_name)),p_template_language,pg_catalog.btrim(p_template_version),
    pg_catalog.coalesce(p_template_variables,'{}'::jsonb),v_contact.provider_address,pg_catalog.btrim(p_idempotency_key),
    pg_catalog.coalesce(p_correlation_id,gen_random_uuid()),p_max_attempts)
  on conflict(idempotency_key) do update set idempotency_key=excluded.idempotency_key
    where whatsapp_outbound_messages.contact_id=excluded.contact_id
      and whatsapp_outbound_messages.candidate_id is not distinct from excluded.candidate_id
      and whatsapp_outbound_messages.requirement_id is not distinct from excluded.requirement_id
      and whatsapp_outbound_messages.purpose=excluded.purpose
      and whatsapp_outbound_messages.template_name=excluded.template_name
      and whatsapp_outbound_messages.template_version=excluded.template_version
  returning id into v_id;
  if v_id is null then raise exception 'WhatsApp outbound idempotency key conflicts with existing message'; end if;
  return v_id;
end;
$$;
...
begin
  ...
  return query with claimable as (
    ...
    where ((o.state='queued' and o.next_attempt_at<=pg_catalog.clock_timestamp())
      or (o.state='sending' and o.lease_expires_at<=pg_catalog.clock_timestamp() and o.provider_message_id is null
        and o.last_error_category is distinct from 'ambiguous'))
      and o.attempt_count<o.max_attempts and (select private.whatsapp_contact_allows_purpose(o.contact_id,o.consent_class))
    order by o.next_attempt_at,o.created_at for update skip locked limit v_batch
  ), claimed as (
    update public.whatsapp_outbound_messages o set state='sending',attempt_count=o.attempt_count+1,lease_owner=v_worker,
...
begin
  update public.whatsapp_outbound_messages set provider_message_id=pg_catalog.btrim(p_provider_message_id),state='sent',
  ...
  where id=p_message_id and state='sending' and lease_owner=pg_catalog.btrim(p_worker_id);
...
begin
  ...
  if v_kind='transient' and v_attempt<v_max and p_retry_at is not null and p_retry_at>pg_catalog.clock_timestamp() then
    update public.whatsapp_outbound_messages set state='queued',next_attempt_at=p_retry_at,lease_owner=null,lease_expires_at=null,
    ...
    update public.whatsapp_outbound_messages set state='failed',failed_at=pg_catalog.clock_timestamp(),lease_owner=null,lease_expires_at=null,
      last_error_category=case when v_kind='ambiguous' then 'ambiguous' else p_error_category end,last_error_code=p_error_code,
    ...
    insert into public.audit_logs(actor_type,action,entity_type,entity_id,source,correlation_id,metadata)
      select 'service','whatsapp.outbound_failed','whatsapp_outbound_message',o.id,'whatsapp',o.correlation_id,
        pg_catalog.jsonb_build_object('failure_kind',v_kind,'error_category',case when v_kind='ambiguous' then 'ambiguous' else p_error_category end,'error_code',p_error_code)
...
returns table(message_id uuid,contact_id uuid,contact_masked text,message_type text,safe_text text,processing_status text,provider_timestamp timestamptz)
language plpgsql stable security definer set search_path = '' as $$
begin
  if not (select private.can_admin_whatsapp()) then raise exception 'WhatsApp Admin access is required'; end if;
  return query select m.id,m.contact_id,'+91XXXXXX'||pg_catalog.right(c.indian_mobile_key,4),m.message_type,m.safe_text,m.processing_status,m.provider_timestamp
...
revoke all on function private.normalize_indian_whatsapp_phone(text),private.can_admin_whatsapp(),
