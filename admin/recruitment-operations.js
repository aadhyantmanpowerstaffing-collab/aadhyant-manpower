(function initializeRecruitmentOperationsModule() {
  ...
  const mutateRecord = async (client, key, row) => {
    ...
    } else if (key === 'recruitmentRequirements') {
      const candidate=window.prompt('Candidate UUID to match to this requirement');if(!candidate)return;
      await call(client,'create_recruitment_application',{p_candidate_id:candidate,p_requirement_id:row.id,p_source_reference:'W3 portal',p_correlation_id:correlation});
  ...
  window.aadhyantRecruitmentOperations=Object.freeze({initialize,definitions,canMutate});
}());
