(function initializeRecruitmentOperationsModule() {
  ...
  const applicationTransitionGraph = Object.freeze({
    ...
    screening: Object.freeze([{ value: 'shortlisted', label: 'Shortlist Candidate' }]),
    shortlisted: Object.freeze([{ value: 'interview', label: 'Move to Interview' }]),
    interview: Object.freeze([{ value: 'selected', label: 'Select Candidate' }, { value: 'rejected', label: 'Reject Candidate' }]),
    selected: Object.freeze([{ value: 'rejected', label: 'Reject Candidate' }])
  });
  const getApplicationTransitions = (stage) => applicationTransitionGraph[stage] || Object.freeze([]);
  const canTransitionApplication = (application, permissions) => Boolean(permissions?.application_mutation && getApplicationTransitions(application?.application_status).length);
  const friendlyTransitionError = (error) => {
    const message=String(error?.message||'');
    if(/unsupported application stage transition/i.test(message))return 'This application changed elsewhere or the selected stage is no longer valid. Refresh Applications and try again.';
    if(/application management access is required|permission|not authorized/i.test(message))return 'You are not authorized to change application stages.';
    if(/application was not found/i.test(message))return 'This application is no longer available. Refresh Applications.';
    if(/application note is too long/i.test(message))return 'The transition reason is too long.';
    return 'The application stage could not be changed. Check the connection and try again.';
  };
  const openApplicationTransition = (client, application) => new Promise((resolve) => {
    const transitions=getApplicationTransitions(application.application_status);const dialog=make('dialog','detail-dialog application-transition-dialog');dialog.setAttribute('aria-labelledby','application-transition-title');
    const heading=make('div','dialog-heading');const copy=make('div');copy.append(make('p','admin-eyebrow','Application pipeline'),make('h2','','Change Application Stage'));copy.querySelector('h2').id='application-transition-title';const close=make('button','dialog-close','×');close.type='button';close.setAttribute('aria-label','Close stage change');heading.append(copy,close);
    const context=make('dl','match-requirement-context');[['Candidate',application.candidate_name],['Requirement',application.requirement_code],['Company',application.company_name],['Role',application.job_role],['Current stage',readable(application.application_status)]].forEach(([label,value])=>{const item=make('div');item.append(make('dt','',label),make('dd','',value||'—'));context.append(item);});
    const form=make('form','application-transition-form');const field=make('div','admin-field');const select=make('select');select.name='nextStage';select.required=true;transitions.forEach(({value,label})=>{const option=make('option','',label);option.value=value;select.append(option);});field.append(make('label','','Next action'),select);const reasonField=make('div','admin-field');const reason=make('textarea');reason.name='reason';reason.maxLength=2000;reason.rows=3;reason.placeholder='Optional transition reason';reasonField.append(make('label','','Reason (optional)'),reason);
    const review=make('section','application-transition-review');const reviewText=make('p');const updateReview=()=>{const choice=transitions.find((item)=>item.value===select.value);reviewText.textContent=`Candidate: ${application.candidate_name}. Current stage: ${readable(application.application_status)}. New stage: ${choice?.label||'—'}.`;};review.append(make('h3','','Review stage change'),reviewText);
    const actions=make('div','dialog-actions');const confirm=make('button','admin-button admin-button-primary','Confirm Change');confirm.type='submit';const cancel=make('button','admin-button admin-button-quiet','Cancel');cancel.type='button';actions.append(confirm,cancel);const message=make('div','admin-message');message.setAttribute('role','status');message.setAttribute('aria-live','polite');form.append(field,reasonField,review,actions,message);dialog.append(heading,context,form);document.body.append(dialog);
    let settled=false;const finish=(changed)=>{if(settled)return;settled=true;dialog.close();dialog.remove();resolve(changed);};const setTransitionMessage=(text,failed=false)=>{message.textContent=text;message.classList.toggle('is-error',failed);};
    select.addEventListener('change',updateReview);form.addEventListener('submit',async(event)=>{event.preventDefault();const allowed=transitions.some((item)=>item.value===select.value);if(!allowed){setTransitionMessage('Select an available stage action.',true);return;}confirm.disabled=true;cancel.disabled=true;setTransitionMessage('Changing application stage…');try{await call(client,'transition_recruitment_application',{p_application_id:application.id,p_to_stage:select.value,p_reason:reason.value.trim()||null,p_internal_notes:null,p_correlation_id:crypto.randomUUID()});finish(true);}catch(error){confirm.disabled=false;cancel.disabled=false;setTransitionMessage(friendlyTransitionError(error),true);}});
    close.addEventListener('click',()=>finish(false));cancel.addEventListener('click',()=>finish(false));dialog.addEventListener('cancel',(event)=>{event.preventDefault();finish(false);});updateReview();dialog.showModal();select.focus();
  });
  ...
  window.aadhyantRecruitmentOperations=Object.freeze({initialize,definitions,canMutate,isRequirementMatchable,isCandidateMatchable,canMatchRequirement,candidateSearchArgs,friendlyMatchError,openCandidatePicker,applicationTransitionGraph,getApplicationTransitions,canTransitionApplication,friendlyTransitionError,openApplicationTransition});
}());
