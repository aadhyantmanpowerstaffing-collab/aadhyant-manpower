...
Job Lead detail is read/view-first and opens on demand through `admin_get_job_lead_detail(uuid)` using only the canonical Requirement UUID from the list row. The latest RPC response, rather than list-row fields, drives the detail modal. Owner and follow-up mutations are deliberately deferred until their reviewed mutation contract is exposed to this UI; no direct base-table writes are permitted. The existing Requirement workspace is extended rather than replaced by a second CRM/navigation tree.

Validation for this slice covers RPC-name/contract usage, UUID-only on-demand detail loading, filter argument mapping, bounded paging, read-only scope, privacy-safe rendering, and the existing Phase-A/W7 regression suites. Human browser review of the local Admin surface remains the next review boundary; no production or NONPROD mutation is part of frontend implementation.

The final Job Leads polish keeps the action column reachable with compact/sticky table treatment and preserves responsive card behavior. Source filtering is a controlled selector containing only the approved recruitment vocabulary. No safe owner-options RPC exists in the installed backend contract, so owner filtering remains a bounded canonical owner-UUID input rather than inventing a direct staff-table lookup; the dedicated Unassigned filter remains available. My Work / Attention is collapsed behind an explicit “Open attention queue” disclosure on the workspace page while retaining all derived attention data.
